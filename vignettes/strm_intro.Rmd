...
out2 <- strm(formula2, id="state", data=Produc, listw= usalw, time=2,trans=NULL,year==1970 | year==1971)
...
m2 <- lm(LNP1000 ~ LNP0090 + POLD00 + POLD90, data = sptdmg)
summary(m2)
```
...
id <- "FIRST_NA_1"
outdf <- createlagvars(data = sptdmg_sf, vars=c(y,xs), id=id, time=time, trans=0)
...
res <- strm(formula, id="FIRST_NA_1", data=sptdmg_long, listw= listwk, time=2,trans=NULL,wide=TRUE)
